  ...
  return [
    {
      ...
      description: "stop driving and diagnose now — a sustained misfire dumps unburned fuel into the catalytic converter and can destroy it",
      impact: 0.9,
      confidence: 0.9,
      infoGain: 0.1,
      cost: 0.4,
    ...
    {
      id: "swap-coil-plug",
      description: "swap the ignition coil and spark plug from the misfiring cylinder with an adjacent cylinder, then rescan — if the misfire follows the part, it's ignition; if it stays with the cylinder, look downstream",
      impact: 0.3,
      confidence: 0.8,
      infoGain: 0.9,
      cost: 0.1,
    ...
    },
    {
      ...
      description: "run a cylinder compression/leak-down test to rule out a mechanical cause (valve, ring, head gasket) before replacing electrical parts",
      impact: 0.5,
      confidence: 0.7,
      infoGain: 0.8,
      cost: 0.3,
...
function misfireDraft(vehicle: VehicleView): FramingResult {
  return {
    ...
    statement: {
      ...
      whyItMatters: "a sustained misfire under load can overheat and destroy the catalytic converter — this is time-sensitive",
      urgency: "high",
    },
    gapType: "causal",
    desiredState: {
      successCriteria: "the misfire DTC does not return after a completed drive cycle under comparable load",
      measurement: "rescan for stored/pending misfire DTCs after the recommended test/repair and a drive cycle",
    },
...
export const misfireCartridge: Cartridge = {
  name: "misfire",
  perception: [
    { dtcConcept: "CylinderMisfire", concept: "CylinderMisfire", as: "symptom", slot: "cylinder-misfire" },
    { pid: ENGINE_LOAD_PID, when: { gt: HIGH_LOAD_PCT }, concept: "HighLoad", as: "condition", slot: "high-load" },
  ],
