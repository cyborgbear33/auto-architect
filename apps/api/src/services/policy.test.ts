...
describe("PolicyService", () => {
  it("does not forbid clear-codes-and-drive when nothing dangerous is proven", async () => {
    ...
    expect(policy.isActionForbidden(evaluation, "clear-codes-and-drive")).toEqual({ forbidden: false });
  });
  ...
  it("forbids clear-codes-and-drive when MisfireUnderLoad is proven (real safety hold, not a UI suggestion)", async () => {
    ...
    expect(evaluation.obligations).toContain("Ought(StopDrivingAndDiagnose(veh_jeep_renegade_2015_latitude))");
  });
