...
function StatusPill({ status }: { status: string }) {
  return (
    <span className={`inline-block rounded-full border px-2 py-0.5 text-xs font-medium ${STATUS_STYLES[status] ?? STATUS_STYLES.open}`}>
      {status}
    </span>
  );
}
...
function VehicleDiagnosis({ vehicleId }: { vehicleId: string }) {
  ...
  const [clearCodesResult, setClearCodesResult] = useState<
    { kind: "allowed"; obligations: string[] } | { kind: "blocked"; message: string; details: unknown } | null
  >(null);

  const recognitionQ = useQuery({ queryKey: ["recognition", vehicleId], queryFn: () => api.getRecognition(vehicleId) });
  const problemsQ = useQuery({ queryKey: ["problems", vehicleId], queryFn: () => api.listProblems(vehicleId) });

  const draftedClasses = new Set(problemsQ.data?.map((p) => p.triggeredByClass).filter(Boolean));
  const undraftedClasses = (recognitionQ.data?.mostSpecific ?? []).filter((c) => !draftedClasses.has(c));

  const createProblem = useMutation({
    mutationFn: (triggeredByClass: string) => api.createDiagnosticProblem({ vehicleId, triggeredByClass }),
  ...
  const clearCodes = useMutation({
    ...
    onSuccess: (result) => setClearCodesResult({ kind: "allowed", obligations: result.obligations }),
    onError: (err) => {
  ...
  return (
    <div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">Proven, not-yet-drafted fault classes</h2>
        {undraftedClasses.length === 0 ? (
          <p className="text-sm text-slate-400">Nothing new to draft — every proven class already has a diagnostic problem below.</p>
        ) : (
          <ul className="space-y-2">
            {undraftedClasses.map((cls) => (
              <li key={cls} className="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 text-sm">
      ...
      <section className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
        ...
        {problemsQ.data?.length === 0 && <p className="text-sm text-slate-400">No diagnostic problems drafted yet.</p>}
        <ul className="space-y-2">
          {problemsQ.data?.map((problem) => (
            <li key={problem.id}>
              ...
              >
                <span>
                  <span className="font-medium text-slate-800">{problem.triggeredByClass ?? "manual"}</span>
      ...
      <section className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">Safety hold demo: clear codes &amp; drive</h2>
        <p className="mb-3 text-xs text-slate-500">
          A real policy gate (LOGOS <code>reason</code>), not a UI suggestion — this <em>blocks</em> when a dangerous fault class is
          currently proven, e.g. <code>MisfireUnderLoad</code>.
        </p>
        <button
          type="button"
        ...
        {clearCodesResult?.kind === "allowed" && (
          <p className="mt-2 rounded-md bg-green-50 px-3 py-2 text-sm text-green-800">
            Allowed. {clearCodesResult.obligations.length > 0 && `Obligations: ${clearCodesResult.obligations.join(", ")}`}
          </p>
        )}
        {clearCodesResult?.kind === "blocked" && (
          <p className="mt-2 rounded-md bg-red-50 px-3 py-2 text-sm text-red-800">Blocked: {clearCodesResult.message}</p>
        )}
      </section>
    </div>
  );
