...
class GatewayConfig:
    api_base_url: str = field(default_factory=lambda: os.environ.get("AUTO_API_BASE_URL", "http://localhost:4100"))
    ...
    obd_baudrate: int | None = field(
        default_factory=lambda: int(os.environ["AUTO_OBD_BAUDRATE"]) if os.environ.get("AUTO_OBD_BAUDRATE") else None
    )
    obd_protocol: str | None = field(default_factory=lambda: os.environ.get("AUTO_OBD_PROTOCOL") or None)
    obd_fast: bool = field(default_factory=lambda: os.environ.get("AUTO_OBD_FAST", "true").lower() != "false")
    poll_interval_seconds: float = field(default_factory=lambda: _env_float("AUTO_POLL_INTERVAL_SECONDS", 5.0))
    pids: tuple[str, ...] = field(default_factory=lambda: _env_list("AUTO_PIDS", DEFAULT_PIDS))
    request_timeout_seconds: float = field(default_factory=lambda: _env_float("AUTO_REQUEST_TIMEOUT_SECONDS", 10.0))
